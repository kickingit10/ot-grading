      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {showWelcome && (
          <div className={`mb-6 px-4 py-3 rounded-xl border flex items-center justify-between animate-slide-in ${
            ts ? 'bg-amber-500/[0.06] border-amber-500/20' : 'bg-indigo-50 border-indigo-200'
          }`}>
            <p className={`text-sm ${ts ? 'text-amber-400' : 'text-indigo-700'}`}>
            ...
            <button onClick={dismissWelcome} className={`text-xs px-2 py-1 rounded ${ts ? 'text-[#9ca3af] hover:bg-white/[0.06]' : 'text-slate-400 hover:bg-slate-100'}`}>Dismiss</button>
          </div>
        )}
        ...
        <div className="flex justify-between items-start mb-6">
          <div>
            <h1 className={`text-3xl font-semibold tracking-tight ${ts ? 'text-[#f0e6d3]' : 'text-slate-900'}`}>Students</h1>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
          {statCards.map((s) => (
            <div key={s.label} className={`px-4 py-3 rounded-xl border ${ts ? 'ts-glass' : 'bg-white border-slate-200'}`}>
              <div className={`text-xl font-semibold tabular-nums ${ts ? 'text-[#f0e6d3]' : 'text-slate-900'}`}>{s.value}</div>
              <div className={`text-xs ${ts ? 'text-[#9ca3af]/60' : 'text-slate-400'}`}>{s.label}</div>
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-wrap gap-1.5 mb-4">
          <button onClick={() => setSchoolFilter(null)}
            className={`px-3 py-1 text-xs font-medium rounded-full transition-all duration-200 ${
              !schoolFilter
                ? ts ? 'bg-amber-500/20 text-amber-400 border border-amber-500/30' : 'bg-indigo-50 text-indigo-700 border border-indigo-200'
                : ts ? 'text-[#9ca3af] hover:bg-white/[0.04] border border-white/[0.08]' : 'text-slate-500 hover:bg-slate-50 border border-slate-200'
            }`}>All ({initialStudents.length})</button>
          ...
          <div className="relative flex-1">
            ...
            <input type="text" placeholder="Search students..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
              className={`w-full pl-9 pr-4 py-2 text-sm border rounded-lg transition-all duration-200 focus:outline-none focus:ring-2 ${
                ts ? 'bg-white/[0.04] border-white/[0.08] text-[#f0e6d3] placeholder:text-[#9ca3af]/50 focus:ring-amber-500/30' : 'bg-white border-slate-200 text-slate-900 placeholder:text-slate-400 focus:ring-indigo-500/40'
              }`} />
          </div>
          <button onClick={() => setShowArchived(!showArchived)}
            className={`px-3 py-2 text-sm rounded-lg border transition-all duration-200 ${
              showArchived ? ts ? 'bg-white/[0.08] border-white/[0.12] text-[#f0e6d3]' : 'bg-slate-100 border-slate-200 text-slate-900'
        ...
        <div className="space-y-6">
          {Object.entries(filteredGroups).length === 0 ? (
            <div className="text-center py-16">
              ...
              <p className={`text-sm ${ts ? 'text-[#9ca3af]' : 'text-slate-500'}`}>{searchTerm ? 'No students match your search' : 'No students yet'}</p>
              {!searchTerm && <Link href="/students/new" className={`text-sm font-medium mt-2 inline-block ${ts ? 'text-amber-400' : 'text-indigo-600'}`}>Add your first student</Link>}
            </div>
          ) : (
              ...
              <div key={schoolName}>
                <button onClick={() => toggleCollapse(schoolName)}
                  className={`flex items-center gap-2 text-xs font-medium uppercase tracking-wider mb-3 transition-colors ${
                    ts ? 'text-[#9ca3af]/70 hover:text-[#9ca3af]' : 'text-slate-400 hover:text-slate-600'
                  }`}>
                      ...
                      return (
                        <Link key={student.id} href={`/students/${student.id}`}
                          className={`block p-5 rounded-xl border transition-all duration-200 ${
                            ts ? 'ts-glass ts-glass-hover' : 'bg-white border-slate-200 hover:shadow-md hover:border-slate-300'
                          }`}>
                          <h3 className={`font-medium mb-3 ${ts ? 'text-[#f0e6d3]' : 'text-slate-900'}`}>{student.first_name} {student.last_name}</h3>
